...
void formatSixDigits(uint32_t value, char *buffer, size_t bufferSize) {
  snprintf(buffer, bufferSize, "%06lu", value % 1000000UL);
}
...
void drawCurrentCode() {
  char codeText[7];
  formatSixDigits(generateTOTP(getRtcSeconds()), codeText, sizeof(codeText));
  drawLargeNumber(codeText);
}
