...
constexpr uint32_t RTC_OVERFLOW_SECONDS = 5UL;
constexpr uint16_t RTC_PERIOD_TOP = RTC_OVERFLOW_SECONDS - 1;
...
uint32_t getRTCSeconds() {
  ...
  ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
    ...
    if ((rtcFlagsSnapshot & RTC_OVF_bm) != 0 &&
        currentCountSnapshot < (RTC_OVERFLOW_SECONDS / 2)) {
...
void sleepForRtcSeconds(uint16_t seconds) {
  const uint32_t wakeAtSeconds = getRTCSeconds() + seconds;

  while (getRTCSeconds() < wakeAtSeconds) {
    const uint32_t currentSeconds = getRTCSeconds();
    const uint16_t currentCount = RTC.CNT;
    const uint32_t secondsRemaining = wakeAtSeconds - currentSeconds;
    const uint16_t secondsUntilOverflow =
        RTC_OVERFLOW_SECONDS - currentCount;

    rtcCompareMatched = false;
    RTC.INTFLAGS = RTC_CMP_bm;

    if (secondsRemaining < secondsUntilOverflow) {
      while (RTC.STATUS & RTC_CMPBUSY_bm) {
      }
      RTC.CMP = static_cast<uint16_t>(currentCount + secondsRemaining);
      while (RTC.STATUS & RTC_CMPBUSY_bm) {
      }
    ...
    while (secondsRemaining < secondsUntilOverflow && !rtcCompareMatched) {
